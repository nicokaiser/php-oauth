...
$test_token = NULL;
if ($token) {
    $test_token = new \OAuth\Consumer($token, $token_secret);
}
...
if ($action == "request_token") {
    ...
    $req_req = \OAuth\Request::fromConsumerAndToken($test_consumer, NULL, "GET", $endpoint, $params);
    $req_req->signRequest($sig_method, $test_consumer, NULL);
    if ($dump_request) {
        header('Content-type: text/plain');
        print "request url: " . $req_req->toUrl() . "\n";
        print_r($req_req);
        exit;
    }
...
<p>Current signing method is: <?php echo $sig_method->getName() ?></p>
<ul>
<?php
...
if ("RSA-SHA1" == $sig_method->getName()) {
    ...
    print "<pre>" . $sig_method->fetchPrivateCert($test_server) . "</pre>\n";
    print "<pre>" . $sig_method->fetchPublicCert($test_server) . "</pre>\n";
}
?>
